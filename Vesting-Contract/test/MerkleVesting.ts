...
describe("Vesting tests", function () {
  it("Testin merkle root claim validation", async function () {
    ...
    const randomUsers = new Array(1024).fill(0).map(() =>
      JSON.stringify({
        address: new Wallet(randomBytes(32).toString("hex")).address,
        tokens: 1,
      })
    );
    ...
    const merkleTree = new MerkleTree(
      randomUsers.concat(
        JSON.stringify({ address: signer.address, tokens: 1 })
      ),
      keccak256,
    ...
    const proof = merkleTree.getHexProof(
      keccak256(JSON.stringify({ address: signer.address, tokens: 1 }))
    );
  ...
  it("VestingVerify tests", async function () {
    ...
    const hash = keccak256(
      ethers.utils.defaultAbiCoder.encode(
        ["address", "uint256", "uin256"],
        [user.address, 1, nonce]
      )
    );
    ...
    expect(
      await vesting
        ...
        .checkClaim(signer.address, 1, nonce, sign)
    ).to.equal(true);
  });
});
