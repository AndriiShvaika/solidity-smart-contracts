...
import {
  DefaultERC20__factory,
  DefaultVesting,
  DefaultVesting__factory,
  ERC20,
} from "../typechain";
...
describe("DefaultVesting", function () {
  ...
  it("Should emit Vest event with correct data when vesting tokens", async function () {
    ...
    let vestTx2 = await vesting
      .vestTokens(address2, defaultVestingBalance.div(2))
      .then((i) => i.wait());
    let vestTx3 = await vesting
      .vestTokens(address3, defaultVestingBalance.div(2))
      .then((i) => i.wait());

    let claimTx2 = await vesting
      .connect(ethers.provider.getSigner(1))
      .claimTokens(defaultVestingBalance.div(4))
      .then((i) => i.wait());
    let claimTx3 = await vesting
      .connect(ethers.provider.getSigner(2))
      .claimTokens(defaultVestingBalance.div(3))
      .then((i) => i.wait());
    ...
    expect(vestingBalance).to.be.equal(
      defaultVestingBalance.sub(balance2).sub(balance3)
    );
