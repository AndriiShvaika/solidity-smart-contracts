...
describe("MerkleVesting", function () {
  ...
  beforeEach(async () => {
    ...
    erc20 = await new ERC20__factory(signer).deploy("Vesting token", "VST");
    vesting = await new MerkleProofVesting__factory(signer).deploy(
      erc20.address
    );
  ...
  it("Should emit Vest event with correct data when vesting tokens", async function () {
    ...
    let vestData1 = ethers.utils.solidityPack(
      ["address", "uint256"],
      [address2, merkleVestingBalance.div(2)]
    );
    let vestData2 = ethers.utils.solidityPack(
      ["address", "uint256"],
    ...
    let merkleTree = new MerkleTree([vestData1, vestData2], keccak256, {
      hashLeaves: true,
      sortPairs: true,
    });

    let vestTx = await vesting
      .vestTokens(merkleTree.getHexRoot())
      .then((i) => i.wait());

    // we need to specify exact data that leaf has in case of off-chain pattern, ...
    let claimTx2 = await vesting
